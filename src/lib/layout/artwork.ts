...
export function sanitizeArtworkSvg(source:string):ArtworkDocument{
  ...
  let count=0,removed=[...parsed.documentElement.attributes].some(attribute=>!['viewBox','width','height','xmlns'].includes(attribute.name)),geometry=0;
  const visit=(element:Element):ArtworkNode|null=>{
    const tag=element.localName;
    if(!TAGS.has(tag)){removed=true;return null;}
    if(++count>MAX_NODES)throw new Error(`SVG exceeds the ${MAX_NODES} element limit.`);
    const attrs:Record<string,string>={};
    for(const attribute of [...element.attributes]){
      const name=attribute.localName;
      if(name==='style'){for(const declaration of attribute.value.split(';')){const separator=declaration.indexOf(':');if(separator<0)continue;const property=declaration.slice(0,separator).trim(),value=declaration.slice(separator+1).trim();if(ATTRS.has(property)&&safePaint(value))attrs[property]=value;else if(property)removed=true;}continue;}
      if(!ATTRS.has(name)||name.startsWith('on')||!safePaint(attribute.value)){removed=true;continue;}
