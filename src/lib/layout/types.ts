...
export type PlaceholderElement = MovableElementBase & { type: 'calligram' };
export type LayoutElement = PageElement | GuidelinesElement | ShapeElement | CurvedTitleElement | PlaceholderElement;
...
export function newElement(type: Exclude<ElementType,'page'>, count:number, page:{width:number;height:number}): LayoutElement {
  ...
  const width=Math.min(type==='guidelines'?150:type==='shape'?40:type==='curved-title'?page.width*.8:72,Math.max(20,page.width-30)); const height=Math.min(type==='guidelines'?180:type==='shape'?35:type==='curved-title'?60:proportional?65:50,Math.max(20,page.height-30));
