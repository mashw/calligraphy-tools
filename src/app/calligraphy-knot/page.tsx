...
// TEMPLATE: Keep this layout consistent across tools (Preview-first + Step cards below).
...
function mapComplexityToLobes(complexity: number) {
  return clamp(Math.round(2 + ((complexity - 1) / 9) * 3), 2, 5);
}

function applySymmetry(points: Pt[], symmetryMode: SymmetryMode, cx: number, cy: number) {
  ...
  if (symmetryMode === 'rotational') {
    ...
    for (let i = 0; i < half; i++) {
      const p = out[i];
      out[i + half] = { x: 2 * cx - p.x, y: 2 * cy - p.y };
    }
    return out;
  }

  return points.map((p, i) => {
    const m = points[(n - i) % n];
...
function buildCalligraphyPolyline(
  rng: () => number,
  ...
  const cx = BOX.w / 2;
  const cy = BOX.h / 2;
  const lobes = mapComplexityToLobes(complexity);
  const sampleCount = clamp(18 + lobes * 4 + pointCountDelta, 18, 36);

  const margin = 20;
  const baseAx = BOX.w * (0.22 + lobes * 0.028 + rng() * 0.05);
  const baseAy = BOX.h * (0.2 + lobes * 0.03 + rng() * 0.05);
  const ax = clamp(baseAx, 36, BOX.w / 2 - margin);
  const ay = clamp(baseAy, 34, BOX.h / 2 - margin);

  const loopSpacing = 0.25 + rng() * 0.35;
  const phase = (rng() - 0.5) * 0.75;
  const phase2 = (rng() - 0.5) * 0.5;
  const wobble = wobbleScale * (0.07 + (complexity / 10) * 0.08);
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    const x = cx
      + ax * (Math.sin(t) + loopSpacing * Math.sin((lobes - 1) * t + phase))
      + ax * wobble * Math.sin((lobes + 1) * t + phase2);
    const y = cy
      + ay * (0.7 * Math.sin(2 * t + phase * 0.6) + 0.45 * loopSpacing * Math.sin(lobes * t - phase))
      + ay * wobble * Math.cos((lobes + 2) * t - phase2);

    pts.push({
      x: clamp(x, margin, BOX.w - margin),
      y: clamp(y, margin, BOX.h - margin),
    });
  }

  const symmetric = applySymmetry(pts, symmetryMode, cx, cy);
  return { sampled: symmetric, lobes };
}

function generateKnotSkeleton(seed: number, symmetryMode: SymmetryMode, complexity: number) {
  const rng = mulberry32(seed || 1);
  const crossingLimit = complexity >= 8 ? 20 : 10;

  let next = buildCalligraphyPolyline(rng, symmetryMode, complexity, 1, 0);
  const nextCrossings = detectCrossings(next.sampled);

  if (nextCrossings.length > crossingLimit) {
    next = buildCalligraphyPolyline(rng, symmetryMode, complexity, 0.5, -4);
  }

  const path = pathFromPoints(next.sampled);
  return { path, sampled: next.sampled, lobes: next.lobes };
}
...
function KnotPage() {
  ...
  const [showCrossingMarkers, setShowCrossingMarkers] = useState(false);
      ...
      <header className="px-6 pt-8 pb-4">
        <div className="max-w-[1120px] mx-auto">
          <h1 className="text-3xl font-semibold tracking-tight">
            Calligraphy Tools <span className="text-indigo-600">— Calligraphy Knot</span>
          </h1>
          <p className="mt-1 text-sm text-slate-600">
            Generate strap-like knots suitable for calligraphy layouts.
          </p>
        </div>
      </header>
        ...
        <div className="max-w-[1120px] mx-auto bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-4">
          <div className="flex flex-wrap items-start gap-3 mb-2">
            ...
            <div className="flex flex-wrap items-center gap-2 ml-auto">
              ...
              <label className="inline-flex items-center gap-2 cursor-pointer shrink-0 px-2 py-1 text-sm rounded-lg border border-slate-300 bg-white">
                <input
                  type="checkbox"
                  checked={showCrossingMarkers}
                  onChange={e => setShowCrossingMarkers(e.target.checked)}
                  className="rounded border-slate-300"
                />
                <span className="text-slate-700">Show crossing markers</span>
              </label>
            </div>
          </div>
          ...
          <div className="relative overflow-x-auto rounded-xl border border-slate-200 bg-slate-300">
            ...
            >
              ...
              {showCrossingMarkers && crossings.map(c => (
