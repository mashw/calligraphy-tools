...
type Strap = {
  ...
  ascNibText: string;
  descNibText: string;
  guideDarkText: string;
  gridDarkText: string;
  titleDarkText: string;
...
function parseDarkness(text: string | undefined, fallback: number) {
  const parsed = Number.parseFloat(text ?? '');
  if (!Number.isFinite(parsed)) return fallback;
  return clamp(parsed, 0, 100);
}

function grayFromDarkness(darkness: number): string {
  const l = clamp(226 - (darkness / 100) * 200, 0, 255);
...
export default function PathGuidesPage() {
  ...
  const [renderMode, setRenderMode] = useState<'design' | 'high' | 'print'>('design');
...
const underCrossings = useMemo(() => {
  ...
  const strapVisuals = (strap: Strap) => {
    const guideDark = parseDarkness(strap.guideDarkText, 75);
    const titleDark = parseDarkness(strap.titleDarkText, 85);
    ...
    if (renderMode === 'print') {
      return {
        ...
        tickColor: '#94a3b8',
        frameColor: 'transparent',
        titleColor: grayFromDarkness(titleDark),
      };
    }

    const thinColor = grayFromDarkness(clamp(guideDark - 15, 0, 100));
    const tickColor = '#93c5fd';
    const boldColor = strap.id === activeStrap?.id ? '#7c3aed' : strap.color;
          ...
          return (
            <g key={`hole-${underId}-${c.id}`}>
              ...
              {renderData.map(({ strap, transformed, guideSet, metrics }) => {
                const visuals = strapVisuals(strap);
                const baselineStroke = renderMode === 'design' ? strap.color : '#111827';
                const baselineStrokeWidth = renderMode === 'print' ? 0.9 : (simplify ? metrics.bandWidthMM : 0.9);
                const titleText = strap.titleText.trim();
                const titleFontSize = clamp(strap.script === 'Copperplate' ? metrics.xMM * 0.5 : metrics.xMM * 0.45, 2.5, 6.5);
                      ...
                      <>
                        <GuideOverlay
                          ...
                          style={{
                            ...
                            bold: visuals.overlayBold,
                            colors: {
                              asc: visuals.ascColor,
                              waist: visuals.waistColor,
                              base: visuals.baseColor,
                              desc: visuals.descColor,
                              thin: visuals.thinColor,
                              bold: visuals.boldColor,
                              tick: visuals.tickColor,
                              frame: visuals.frameColor,
                            },
                            grid: {
                              thin: visuals.overlayThin,
                        ...
                        />
                        {titleText.length > 0 && transformed.length > 1 && (
                          <>
                            <defs>
                              <path id={`title-path-${strap.id}`} d={pathD(transformed)} />
                            </defs>
                            <text
                              fill={visuals.titleColor}
                              fontSize={titleFontSize}
                              dominantBaseline="middle"
                              dy={`${-metrics.xMM * 0.15}`}
                              pointerEvents="none"
                            >
                              <textPath href={`#title-path-${strap.id}`} startOffset="50%" textAnchor="middle">
                                {titleText}
                              </textPath>
                            </text>
                          </>
      ...
      <section className="px-6 py-5 max-w-[1120px] mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button onClick={() => setRenderMode('design')} className={`px-2 py-1 text-sm rounded-lg border ${renderMode === 'design' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-slate-300 bg-white'}`}>Design mode</button>
            <button onClick={() => setRenderMode('high')} className={`px-2 py-1 text-sm rounded-lg border ${renderMode === 'high' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-slate-300 bg-white'}`}>High contrast</button>
            <button onClick={() => setRenderMode('print')} className={`px-2 py-1 text-sm rounded-lg border ${renderMode === 'print' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-slate-300 bg-white'}`}>Print mode</button>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
