...
export default function GuidelinesPage() {
  ...
  const toolbarButtonClass = 'px-2 py-1 text-sm rounded-lg border border-slate-300 bg-white hover:bg-slate-50 hover:border-slate-400 hover:text-slate-900 transition-colors';
  const toolbarExportButtonClass = 'px-3 py-1.5 text-sm rounded-lg border border-slate-300 bg-white hover:bg-slate-50 hover:border-slate-400 hover:text-slate-900 transition-colors';
  const toolbarPrimaryButtonClass = 'px-3 py-1.5 text-sm rounded-lg text-white bg-indigo-600 hover:bg-indigo-500 transition-colors';
      ...
      <section className="px-6">
        <div className="max-w-[1120px] mx-auto bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => adjustZoom('out')} className={toolbarButtonClass}>
                –
              </button>
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => adjustZoom('in')} className={toolbarButtonClass}>
                +
              </button>
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={resetView} className={toolbarButtonClass}>
                Reset view
              </button>
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={goToTop} className={toolbarButtonClass}>
                Top
              </button>

              <button type="button" onMouseDown={e => e.preventDefault()} onClick={downloadSVG} className={`ml-2 ${toolbarExportButtonClass}`}>
                SVG
              </button>
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={downloadPDF} className={toolbarExportButtonClass}>
                PDF
              </button>
              <button type="button" onMouseDown={e => e.preventDefault()} onClick={printToScale} className={toolbarPrimaryButtonClass}>
                Print
              </button>
            </div>
          </div>
