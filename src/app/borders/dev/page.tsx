...
export default function BorderDevPage() {
  const [paths, setPaths] = useState(BUILT_INS), [selected, setSelected] = useState(1), [options, setOptions] = useState(DEFAULTS), [reverse, setReverse] = useState(false), [construction, setConstruction] = useState(false), [message, setMessage] = useState<string | null>(null);
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-7 text-slate-900 sm:px-6">
    ...
    <div className="mx-auto grid max-w-[1500px] gap-5 lg:grid-cols-[300px_minmax(0,1fr)]"><BorderDevControls options={options} setOptions={setOptions} presets={paths} selected={selected} onSelect={setSelected} reverse={reverse} onReverse={setReverse} construction={construction} onConstruction={setConstruction} onUpload={upload} message={message} /><BorderDevStage d={paths[selected].d} closed={paths[selected].closed} reverse={reverse} options={options} showConstruction={construction} /></div>
  </main>;
}
