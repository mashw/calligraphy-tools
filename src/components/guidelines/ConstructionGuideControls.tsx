...
export default function ConstructionGuideControls({ script, value, onChange, compact = false }: { script: ScriptId; value?: ConstructionGuideSettings; onChange: (value: ConstructionGuideSettings) => void; compact?: boolean }) {
  ...
  return <div className={compact ? 'space-y-2' : 'space-y-3'}>
    {rows.map(([key, label, tip]) => {const appearanceKey=key==='upper'?'upperAppearance':'lowerAppearance';return <div key={key} className="space-y-1.5"><label className="flex items-center justify-between gap-3 text-sm text-slate-700"><span className="flex items-center gap-1.5">{label}<span title={tip} aria-label={tip} className="cursor-help text-xs text-slate-400">ⓘ</span></span><input type="checkbox" checked={settings[key]} onChange={event => onChange({ ...settings, [key]: event.target.checked })} className="accent-indigo-600" /></label><label className="block space-y-1 pl-4 text-xs font-medium text-slate-600">Appearance<select disabled={!settings[key]} className="w-full rounded border border-slate-300 bg-white px-2 py-2 text-sm text-slate-700 disabled:bg-slate-100 disabled:text-slate-400" value={settings[appearanceKey]} onChange={event => onChange({ ...settings, [appearanceKey]: event.target.value as ConstructionGuideAppearance })}><option value="dashed">Dashed line</option><option value="dots">Dots</option></select></label></div>})}
    {(settings.upperAppearance === 'dots' || settings.lowerAppearance === 'dots') && <label className="block space-y-1 text-xs font-medium text-slate-600">Dot every <span className="flex items-center gap-2"><input type="number" min={1} max={12} step={1} className="w-20 rounded border border-slate-300 px-2 py-1.5 text-sm" value={settings.dotEvery} onChange={event => onChange({ ...settings, dotEvery: Math.max(1, Math.min(12, Math.round(Number(event.target.value) || 3))) })} /> uprights</span><span className="block font-normal text-slate-500">Starting from the first upright.</span></label>}
