...
export default function CurvedTitleRenderer({box,settings,idPrefix}:{box:{w:number;h:number};settings:CurvedTitleSettings;idPrefix:string}) {
  const model=buildCurvedTitleModel(box,settings),pageClip=`${idPrefix}-page`,topClip=`${idPrefix}-top`,bottomClip=`${idPrefix}-bottom`,style={thin:.38,bold:.65,colors:{thin:'#111827',bold:'#111827',tick:'#e2e8f0',frame:'transparent'}};
  return <g pointerEvents="none"><defs><clipPath id={pageClip}><rect width={box.w} height={box.h}/></clipPath><clipPath id={topClip}><path d={model.top.clip}/></clipPath><clipPath id={bottomClip}><path d={model.bottom.clip}/></clipPath></defs><g clipPath={`url(#${pageClip})`}>
