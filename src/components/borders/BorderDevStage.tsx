...
export default function BorderDevStage({ d, closed, reverse, options, showConstruction, singleLeaf = false, componentStudy = false }: { d: string; closed: boolean; reverse: boolean; options: AcanthusOptions; showConstruction: boolean; singleLeaf?: boolean; componentStudy?: boolean }) {
  ...
  const geometry = useMemo(() => componentStudy ? generateComponentStudy(options) : singleLeaf ? generateSingleLeaf(options) : guide ? generateAcanthus(guide, options) : null, [componentStudy, guide, options, singleLeaf]);
  ...
  return <div className="relative min-h-[650px] overflow-hidden rounded-xl border border-slate-300 bg-[#fffef9] shadow-inner">
    <svg className="h-full min-h-[650px] w-full" viewBox="0 0 600 430" aria-label="Procedural acanthus geometry preview">
      ...
      {showConstruction && <g fill="none" vectorEffect="non-scaling-stroke">
        {!singleLeaf && !componentStudy && <path d={d} stroke="#e11d48" strokeWidth="1.2" strokeDasharray="4 3" />}
        {!componentStudy && stations.map((f, i) => <g key={i}><circle cx={f.point.x} cy={f.point.y} r="1.25" fill={Math.abs(f.curvature) > .055 ? '#f97316' : '#2563eb'} /><path d={`M ${f.point.x} ${f.point.y} l ${f.tangent.x * 6} ${f.tangent.y * 6}`} stroke="#2563eb" strokeWidth=".65"/><path d={`M ${f.point.x} ${f.point.y} l ${f.normal.x * 4} ${f.normal.y * 4}`} stroke="#10b981" strokeWidth=".55"/></g>)}
