...
import { generateAcanthus } from '@/lib/borders/acanthus/generate';
...
export default function BorderDevStage({ d, closed, reverse, options, showConstruction }: { d: string; closed: boolean; reverse: boolean; options: AcanthusOptions; showConstruction: boolean }) {
  ...
  const geometry = useMemo(() => guide ? generateAcanthus(guide, options) : null, [guide, options]);
  ...
  return <div className="relative min-h-[650px] overflow-hidden rounded-xl border border-slate-300 bg-[#fffef9] shadow-inner">
    <svg className="h-full min-h-[650px] w-full" viewBox="0 0 600 430" aria-label="Procedural acanthus geometry preview">
      ...
      {geometry?.strokes.map((stroke, i) => <path key={i} d={stroke.d} fill="none" stroke={stroke.role === 'shading' ? '#42403b' : '#111'} strokeWidth={stroke.role === 'outline' ? 1.15 : stroke.role === 'shading' ? .52 : .72} strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />)}
      {showConstruction && <g fill="none" vectorEffect="non-scaling-stroke">
        <path d={d} stroke="#e11d48" strokeWidth="1.2" strokeDasharray="4 3" />
        ...
        {geometry?.construction.map((m, i) => m.kind === 'root' ? <circle key={i} cx={m.a.x} cy={m.a.y} r="2.2" fill="#7c3aed" /> : m.kind === 'tight' ? <circle key={i} cx={m.a.x} cy={m.a.y} r="5" fill="#ef444455" stroke="#ef4444" /> : <path key={i} d={`M ${m.a.x} ${m.a.y} L ${m.b?.x} ${m.b?.y}`} stroke="#8b5cf6" strokeWidth=".5" strokeDasharray="2 2" />)}
