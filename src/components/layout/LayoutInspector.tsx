...
export default function LayoutInspector({ element, page, onChange }: { element: LayoutElement; page: PageElement; onChange: (element: LayoutElement) => void }) {
  ...
  const quickSize = (axis: 'width' | 'height', fraction: 0.5 | 1) => { if (element.type === 'guidelines') onChange({ ...element, frame: sizeToPageContent(element.frame, element.settings.margins, pageRect, axis, fraction) }); };
  ...
  return <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
    ...
    <div className="mt-4 space-y-3 xl:flex xl:flex-wrap xl:items-start xl:gap-4 xl:space-y-0">
      {element.type === 'page' ? <>
        ...
        {element.type === 'guidelines' ? <GuidelinesSettingsPanel value={element.settings} onChange={settings => onChange({ ...element, settings })} /> : <DisclosureSection title="Element settings" className={sectionClass}><p className="text-sm text-slate-600">Settings will be added in a later pass.</p></DisclosureSection>}
      </>}
    </div>
  </section>;
}
