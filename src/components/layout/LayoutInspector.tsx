  ...
  return <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
    ...
    <div className="mt-4 space-y-3 xl:flex xl:flex-wrap xl:items-start xl:gap-4 xl:space-y-0">
      {element.type === 'page' ? <>
        ...
        {element.type === 'guidelines' ? <GuidelinesSettingsPanel key={element.id} value={element.settings} onChange={settings => onChange({ ...element, settings })} paddingMM={element.paddingMM} onPaddingChange={paddingMM => onChange({ ...element, paddingMM })} /> : element.type === 'shape' ? <>
