...
export default function LayoutInspector({ element, page, textFitEntry, onChange }: { element: LayoutElement; page: PageElement; textFitEntry: GuidelinesTextFitEntry | null; onChange: (element: LayoutElement) => void }) {
  ...
  const patchFrame = (key: 'x' | 'y' | 'width' | 'height', value: number) => {
    ...
    if(element.type==='artwork'&&element.settings.lockProportions&&(key==='width'||key==='height')){const ratio=element.frame.width/element.frame.height,cx=element.frame.x+element.frame.width/2,cy=element.frame.y+element.frame.height/2;if(key==='width')frame={...frame,height:value/ratio,y:cy-value/ratio/2};else frame={...frame,width:value*ratio,x:cx-value*ratio/2};}
  ...
  const quickSize = (axis: 'width' | 'height', fraction: 0.5 | 1) => {
    ...
    if(element.type==='artwork'&&element.settings.lockProportions){const ratio=element.frame.width/element.frame.height,cx=element.frame.x+element.frame.width/2,cy=element.frame.y+element.frame.height/2;if(axis==='width')frame={...frame,height:frame.width/ratio,y:cy-frame.width/ratio/2};else frame={...frame,width:frame.height*ratio,x:cx-frame.height*ratio/2};}
  ...
  return <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
    ...
    <SettingsAccordion sessionKey={element.id} defaultTitle={element.type === 'page' ? 'Page' : 'Position & Size'}>
      {element.type === 'page' ? <>
        ...
        {element.type === 'artwork' ? <ArtworkSettingsPanel element={element} onChange={onChange} /> : element.type === 'guidelines' ? <GuidelinesSettingsPanel key={element.id} value={element.settings} onChange={settings => onChange({ ...element, settings, frame: element.allowPartialGuidelines ? element.frame : { ...element.frame, height: getNearestCompleteGuidelinesHeight(settings, element.frame.height) } })} paddingMM={element.paddingMM} onPaddingChange={paddingMM => onChange({ ...element, paddingMM })} allowPartialGuidelines={element.allowPartialGuidelines} onAllowPartialGuidelinesChange={allowPartialGuidelines => onChange({ ...element, allowPartialGuidelines, frame: allowPartialGuidelines ? element.frame : { ...element.frame, height: getNearestCompleteGuidelinesHeight(element.settings, element.frame.height) } })} fitText={element.fitText} onFitTextChange={fitText=>onChange({...element,fitText})} fitEntry={textFitEntry!} textMode={element.textMode} plannedLines={element.plannedLines} rightAlignMode={element.rightAlignMode} onPlanningChange={patch=>onChange({...element,...patch})} /> : element.type === 'shape' ? <>
