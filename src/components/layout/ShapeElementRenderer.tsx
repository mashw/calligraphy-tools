import { PAGE_BACKGROUND, shapeVisibleBox } from '@/lib/layout/shape';
...
export default function ShapeElementRenderer({ element, frame }: { element: ShapeElement; frame: Frame }) {
  ...
  const visible = shapeVisibleBox(frame, settings.paddingMM);
  ...
  const strokeInset = Math.min(borderWidth / 2, visible.width / 2, visible.height / 2);
  const artworkBox = { x: visible.x + strokeInset, y: visible.y + strokeInset, width: Math.max(0, visible.width - strokeInset * 2), height: Math.max(0, visible.height - strokeInset * 2) };
  ...
  return <g transform={`translate(${frame.x} ${frame.y})`}>
    <rect width={frame.width} height={frame.height} fill={PAGE_BACKGROUND} />
    {ellipse
    ...
    {settings.appearance === 'reserve' && <rect data-no-export="true" width={frame.width} height={frame.height} fill="#f8fafc" fillOpacity=".28" stroke="#94a3b8" strokeWidth="1" strokeDasharray="3 2" vectorEffect="non-scaling-stroke" />}
  </g>;
}
